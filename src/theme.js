...
(() => {
    ...
    function addLightElem() {
        if (localStorage.theme === 'light')
            socialItemAll.forEach((item) => item.classList.remove('social_light'));
        if (localStorage.theme === 'light dark')
            socialItemAll.forEach((item) => item.classList.add('social_light'));
    }
    //3
    function changeImgTheme() {
        if (localStorage.theme === 'light')
            imgTheme.classList.remove('sun_img');
        if (localStorage.theme === 'light dark')
            imgTheme.classList.add('sun_img');
    }
    //4
    function burgerTheme() {
        if (localStorage.theme === 'light')
            imgThemeBurger.forEach((item) => item.classList.remove('sun_img'));
        if (localStorage.theme === 'light dark')
            imgThemeBurger.forEach((item) => item.classList.add('sun_img'));
    }
    //5
